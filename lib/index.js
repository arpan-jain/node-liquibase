...
class Liquibase {
	 ...
	 * @param {Object} input default parameters for Liquibase
	 * @param {String} input.liquibase - Absolute path to your Liquibase executable.
	 * @param {String} input.changeLogFile - Absolute path to your Change Log File
	 * @param {String} input.url - JDBC connection string
	 * @param {String} input.username - username
	 * @param {String} input.password - password
	 * @param {String} input.liquibaseProLicenseKey - Your Liquibase Pro License key
	 * @param {String} input.classpath - Absolute path to your JDBC driver jar file
	 *
	 * @example
	 * ```javascript
	...
	run(action = 'update', params = '') {
		return this._exec(`${this._command} ${action} ${params}`,{maxBuffer: 1024 * 1024});
	}
